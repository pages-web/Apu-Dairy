...
const ThhirdItem = ({ posts }: { posts: ICmsPost }) => {
  return (
    <div className="flex flex-col items-center px-1">
      {posts.thumbnail?.url && (
        <div className="relative w-full aspect-square overflow-hidden rounded-3xl">
          <Image
            src={`https://apudairy.api.erxes.io/api/read-file?key=${posts.thumbnail.url}`}
            alt={posts.title}
            fill
      ...
      <div className="flex flex-col w-full max-w-[500px] gap-2 text-center mt-3">
        <div className="text-[18px] font-normal font-weigth-400 text-black z-10 font-sf-pro-rounded mt-2">
          {posts.title}
        </div>
        <div className="flex items-center gap-4 text-gray-700 text-sm font-medium mb-3 justify-center">
